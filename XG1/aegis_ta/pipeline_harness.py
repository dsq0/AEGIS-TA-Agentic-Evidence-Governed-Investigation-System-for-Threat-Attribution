...
from aegis_ta.discriminative import path_discriminative_weight, reweight_graph_channel
            ...
            out.append(
                {
                    ...
                    "source_path": (s.get("source_path") or "")[:300],
                    "split": s.get("split"),
    ...
    def run(
        ...
        while True:
            ...
            if action == "ingest_bundle":
                ...
                mode = "candidate" if cand else "global"
                rag = self.retrieval_agent.retrieve(
                    mode,
                    fpath,
                    index_bundle,
                    embed_fn,
                    ...
                    candidate_actors=cand,
                )
                rs = rag["scores"]
                state.channel_scores["report"] = dict(rs)
                state.retrieval_evidence = list(rag["evidence"])
                state.phases_done.add("run_retrieval_agent")
                self.trace.log(step, "RetrievalAgent", "rag", rag.get("trace_summary", ""))
                self._evidence_add(
                    step,
                    "RetrievalAgent",
                    "retrieval_evidence",
                    {
                        "mode": mode,
                        "trace_summary": rag.get("trace_summary", ""),
                ...
                gs, eg = self.graph_agent.infer_paths(
                    fpath,
                    index_bundle,
                    embed_fn,
                    qdist,
                ...
                rag_c = self.retrieval_agent.retrieve(
                    "contrast",
                    fpath,
                    index_bundle,
                    embed_fn,
                ...
                sc = rag_c["scores"]
                state.retrieval_contrast_evidence = list(rag_c["evidence"])[:8]
                w = float(self.hw.get("contrast_merge_weight", 0.14))
                rep = dict(state.channel_scores.get("report", {}))
                for a, v in sc.items():
                    rep[a] = rep.get(a, 0.0) + w * float(v)
                state.channel_scores["report"] = rep

                pta_g = state.channel_scores.get("_p_t_given_a") or p_mitre_code_given_threat_actor
                g_con = self.graph_agent.contrast_paths(a1, a2, pta_g, paths=state.graph_paths)
                rho = float(self.hw.get("contrast_graph_rho", 0.08))
                bonus_by_actor: Dict[str, float] = defaultdict(float)
                for p in g_con.get("paths") or []:
                    act = p.get("target_actor")
                    if not act:
                        continue
                    bonus_by_actor[act] += path_discriminative_weight(p, pta_g)
                gs_map = dict(state.channel_scores.get("graph", {}))
                for a, mass in bonus_by_actor.items():
                    gs_map[a] = gs_map.get(a, 0.0) + rho * float(mass)
                state.channel_scores["graph"] = gs_map

                self._fuse_rerank_state(state, ev, fpath, index_bundle, embed_fn)
                state.phases_done.discard("fuse_rerank")
                state.phases_done.add("fuse_rerank")
                ...
                self.trace.log(
                    ...
                    "RetrievalAgent",
                    "actor_contrast",
                    f"{a1} vs {a2}, margin={margin:.4f}; report+graph recontrast+fuse; {rag_c.get('trace_summary', '')}",
                )
                self._evidence_add(
                    step,
                    "RetrievalAgent",
                    "actor_contrast_evidence",
                    {
                        "pair": [a1, a2],
                        "margin": float(margin),
                        "trace_summary": rag_c.get("trace_summary", ""),
                ...
                self._evidence_add(
                    ...
                    "contrast_paths",
                    {
                        "trace_summary": g_con.get("trace_summary", ""),
                        "n_ranked_paths": len(g_con.get("paths") or []),
                        "pair": [a1, a2],
                        "contrast_graph_rho": rho,
                        "contrast_graph_bonus_by_actor": dict(bonus_by_actor),
                    },
                )
                ...
                gs, eg = self.graph_agent.expand_paths(
                    fpath,
                    index_bundle,
                    embed_fn,
                    qdist,
        ...
        softmax_conf = softmax_over_dict(governed)
            ...
            top_k_payload.append(
                {
                    ...
                    "governed_attribution_score": float(governed.get(actor, 0.0)),
                    "confidence_softmax": float(softmax_conf.get(actor, 0.0)),
                    "confidence": float(softmax_conf.get(actor, 0.0)),
        ...
        return HarnessResult(
            ...
            state_summary={
                ...
                "audit": audit,
                "confidence_definition": "confidence / confidence_softmax = softmax(governed_attribution_score) over actors; not calibrated P(true actor).",
            },
        )
