...
class PlannerConfig:
    ...
    disable_actor_contrast: bool = False
...
class RulePlanner:
    ...
    def __init__(self, cfg: PlannerConfig | None = None, runtime: dict | None = None) -> None:
        ...
        for k in ("disable_actor_contrast", "disable_graph_expansion"):
            if k in rt:
                setattr(self.cfg, k, bool(rt[k]))
    ...
    def plan(self, state: "AttributionState", step: int) -> Action:
        if self.cfg.monolith_bundle:
            ...
            if not self.cfg.disable_graph_expansion and float(state.uncertainty.get("verifier_suggest_expand_graph", 0.0) or 0.0) > 0:
            ...
            if not self.cfg.disable_actor_contrast and not state.retrieval_contrast_evidence and (
                suggest_ct
                or (margin < self.cfg.margin_threshold and ent > self.cfg.entropy_threshold)
            ):
                return "actor_contrast_retrieval"
            if not self.cfg.disable_graph_expansion and len(state.graph_paths) < self.cfg.min_graph_paths and state.graph_expand_count < self.cfg.max_graph_expansions:
                return "expand_graph"
